...
private:
    ...
    /**
   * description:   retourne un objet de type Tableau<std::string> qui contient
   * 							 une liste de string qui n'existent pas dans l'objet courant.
   *    						 Cette fonction ne doit pas modifier l'objet courant.

   * @param         une référence vers un objet de type  Tableau<std::string> qui
   * 							 contient plusieurs éléments de type std::string.

  * @return  		   un objet de type Tableau<std::string> contenant des
   * 				  String de l'objet passe en parametre qui n'existent pas dans l'objet courant.
   */

    Tableau<std::string> retournerListeFavoris(const Tableau<std::string> &liste) const;
};
...
Tableau<std::string> Membre::suggestion(const Membre &autre) const {
    ...
    Tableau<std::string> listeFavoris = retournerListeFavoris(lesFavorisAutre);
    return listeFavoris;
}
...
Tableau<std::string> Membre::suggestion(const Tableau<Membre> &com) const {
    ...
    Tableau<std::string> lesFavoris;

    for (int i = 0; i < com.taille(); i++) {

        for (int j = 0; j < com[i].favs.taille(); j++) {

            lesFavoris.ajouter(com[i].favs[j]);

            lesFavoris.unique();
        }
    }
    Tableau<std::string> listeFavoris = retournerListeFavoris(lesFavoris);
